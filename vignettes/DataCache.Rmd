...
cache.dir <- 'cache'
```
```{r createDir, echo=FALSE, warning=FALSE, message=FALSE}
# devtools::install() will not work without this. devtools::build_vignettes() does.
...
```{r firstCache}
(cache.date1 <- data.cache(loadWeatherData, cache.dir=cache.dir))
head(weather.ALB)
```
...
```{r cacheInfo}
cache.info(cache.dir=cache.dir)
```
...
cinfo <- cache.info(cache.dir=cache.dir)
...
```{r updatingCache}
Sys.sleep(60)
(cache.date2 <- data.cache(loadWeatherData, nMinutes(1), cache.dir=cache.dir))
(cache.date3 <- data.cache(loadWeatherData, nMinutes(1), cache.dir=cache.dir))
```
...
```{r jfkCache}
data.cache(loadWeatherData, cache.name='NRT', station_id='NRT', cache.dir=cache.dir)
head(weather.NRT)
cache.info(cache.name='NRT', cache.dir=cache.dir)
```
